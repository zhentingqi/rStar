...
def eval_exp(dataset_name: str):
    ...
    for example_id in tqdm(example_ids):
        # try:
        right = False
        for answer_sheets_dir in [
        ]:
            try:
                dta = eval_single_item_from_answer_sheets(example_id, answer_sheets_dir, evaluator)
                if dta["correctness_1"] or dta["correctness_1_2"] or dta["correctness_2"]:
                    data_list.append(1)
                    right = True
                    break
            except:
