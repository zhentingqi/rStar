...
def eval_single_item_from_answer_sheets(example_id, answer_sheets_dir, evaluator: Evaluator, vote_pool_dir=None, num_votes=-1):
    ...
    data_1 = read_json(os.path.join(answer_sheets_dir, example_id.replace(" - Answer", f" - Rollout {num_votes-1} Solutions") + ".json"))
    data_1, _ = extract_trace(data_1, num_votes) # data_1 + data_2
...
def eval_exp(exp_dir: str, dataset_name: str, num_votes: int = -1):
    ...
    example_ids = [f.replace(".json", "") for f in os.listdir(answer_sheets_dir) \
                   if f.endswith(" - Answer.json") and \
                    True]
                    # int(f.replace(" - Answer.json", "").replace("Question ", "")) < 96]
